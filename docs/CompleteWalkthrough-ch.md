...
|---|---|---|---|
| **CAGR**（年化复合收益率） | **7.73 %** | 权益曲线年化复合收益。`CAGR = (End/Start)^(1/years) − 1`。 | 与买入持有相比，逆向定投在**强牛年收益较低**（持有较少），在**回撤段回撤较浅**（持有更多）。7.73 %/年（横跨 2022 熊市 + 2023–2026 复苏）是这组参数的真实年化。**这不是"好"或"坏"——你的风险偏好、收入、人生阶段决定了哪个更适合你。** |
| **Sharpe**（夏普比率） | **0.04** | 单位波动率对应的超额收益（此处为日收益年化）。 | 逆向定投系统性地在下跌市场加仓，**波动通常低于 buy-and-hold**，但**均值收益也更低**——Sharpe 才是诚实的对比。多年窗口下 Sharpe 接近 0，意味着这个窗口内该策略的风险调整后收益**仅略优于持币**；它只是这组参数在这个窗口的**真实数字**——不是"好"或"坏"的标签。要和 MaxDrawdown、CAGR 一起读，由你判断是否匹配你的风险偏好。 |
| **MaxDrawdown**（最大回撤） | **−18.97 %** | 权益曲线峰谷最大跌幅。 | 这是逆向定投相对买入持有**最显著的差异**：同窗口内回撤更浅。QQQM 自身从 ≈165.24（2022-01-03）跌到 ≈107.00（2022-12-28），峰谷 ≈ **−35.3 %**；策略权益在同一窗口峰谷 **−18.97 %**。这就是公式在卖的回撤保护——**真实存在，大约减半**。 |
...
- **诚实的解读**：逆向定投**并不能躲过** 2022 回撤——它比固定权重**更深度参与**了回撤，赌的是随后的复苏（2023）足够大来补偿。2023 复苏中，策略权益曲线**重新站上 $10,000**（2023-06-22：$10,108.62），随后一路复利到 2026-08-28 的 **$14,356.25**。这笔交易的代价与收益都写在数字里：**更浅的回撤（−18.97 % vs −35.3 %）对应更低的 Sharpe（0.04）和低于标的 ETF 全周期收益的 CAGR。**数字就摆在这里——**好还是不好，取决于你**：你的风险偏好、你的收入节奏、你处于人生的哪个阶段。Quant.Infra.Net 只是工具，它给出数字，判断由你做。

> **不要用单年盈亏下结论。**策略的表现是**全周期**属性：回撤行为 + 复苏行为 + 换手成本，三者合一。而"好不好"本身是个**主观判断**——取决于你的风险偏好、收入、人生阶段。工具给出数字，人自己做判断。

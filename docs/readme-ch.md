...
| Sharpe | **0.04** | 与买入持有是另一种风险/收益取舍——不分好坏，取决于你的风险偏好、收入与人生阶段 |
| 胜率 | **53.3%** | |
| 交易次数 | **673** | 每日再平衡决策 |

> **这是一次对比，而不是好坏判断。** 逆向定会在价格低于 SMA200 时加仓、高于时减仓。在同一窗口里，它呈现的是**另一种**风险/收益画像（更浅的回撤、较低的收益）。**两者并不存在客观的好坏**——哪个更适合，取决于*你*的风险偏好、你的收入、你人生的阶段。Quant.Infra.Net 只是工具：它给出数字，**判断由你来做**。

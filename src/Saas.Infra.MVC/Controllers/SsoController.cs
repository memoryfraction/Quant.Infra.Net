...
namespace Saas.Infra.MVC.Controllers
{
	...
	public class SsoController : ControllerBase
	{
		...
		/// <summary>
		/// 初始化 <see cref="SsoController"/> 类的新实例。
		/// </summary>
		/// <param name="config">应用程序配置实例，用于读取JWT配置。</param>
		/// <param name="logger">日志记录器实例，用于记录操作日志。</param>
		/// <exception cref="ArgumentNullException">
		/// 当 <paramref name="config"/> 或 <paramref name="logger"/> 为null时抛出。
		/// </exception>
		...
		/// <summary>
		/// 处理用户登录请求并生成JWT令牌。
		/// 验证用户凭据后，生成包含用户信息的JWT访问令牌和刷新令牌。
		/// </summary>
		/// <param name="request">包含用户凭据的登录请求对象。</param>
		/// <returns>
		/// 成功时返回200 OK和JWT令牌信息；
		/// 凭据无效时返回401 Unauthorized；
		/// 请求无效时返回400 Bad Request；
		/// 服务器错误时返回500 Internal Server Error。
		/// </returns>
		/// <exception cref="ArgumentNullException">
		/// 当 <paramref name="request"/> 为null时抛出。
		/// </exception>
